...
import { ActivityIndicator, Alert, Linking, Platform, Text, TouchableOpacity, View, Vibration } from 'react-native';
...
export default function PassengerHireTrackingScreen({ navigation, route }) {
  ...
  const triggerPassengerArrivalAlert = useCallback(() => {
    const announcementKey = String(requestId || '');
    ...
    try {
      Speech.stop();
      Speech.speak('Your hire driver has arrived at the pickup point.', {
        rate: 0.95,
        pitch: 1.0,
        language: 'en',
      });
