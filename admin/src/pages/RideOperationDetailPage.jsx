...
export default function RideOperationDetailPage() {
  ...
  const refreshShareLink = async () => {
    ...
    try {
      const { data } = await axios.get(`${BASE_URL}/api/admin/rides/${rideId}/authority-share`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const createAuthorityShare = async () => {
    if (!token || !rideId) return
    setShareBusy(true)
    setShareMessage('')
    setError('')
    try {
      const note = window.prompt('Optional note for this authority share (e.g. ZRP Harare Central)', shareLink?.recipientNote || '')
      if (note === null) {
        setShareBusy(false)
        return
      }
      const { data } = await axios.post(
        `${BASE_URL}/api/admin/rides/${rideId}/authority-share`,
      ...
      if (url && navigator?.clipboard?.writeText) {
        try {
          ...
          setShareMessage('Authority share link created and copied to clipboard.')
        } catch {
          setShareMessage('Authority share link created. Copy it from the box below.')
        }
      } else {
        setShareMessage('Authority share link created. Copy it from the box below.')
      }
  ...
  const revokeAuthorityShare = async () => {
    if (!token || !rideId || !shareLink) return
    if (!window.confirm('Revoke the authority live-share link for this trip?')) return
    setShareBusy(true)
    setShareMessage('')
    setError('')
    try {
      await axios.delete(`${BASE_URL}/api/admin/rides/${rideId}/authority-share`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      setShareLink(null)
      setShareMessage('Authority share link revoked.')
    } catch (err) {
      setError(err?.response?.data?.error || err?.message || 'Failed to revoke authority share link')
    } finally {
      setShareBusy(false)
    }
  }
    ...
    <section className="space-y-3">
      ...
      <div className="border border-rose-200 bg-rose-50 px-4 py-3">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wide text-rose-700">Authority live share</p>
            <p className="mt-1 text-xs text-rose-800/80">
              Create a public tracking page authorities can open without admin login. Updates every 5 seconds while the link is active.
            </p>
            {shareLink?.url ? (
              <div className="mt-3 space-y-1">
                <p className="break-all text-xs font-medium text-slate-800">{shareLink.url}</p>
                <p className="text-[11px] text-slate-600">
                  Expires: {formatDateTime(shareLink.expiresAt)}
                  {shareLink.recipientNote ? ` · Note: ${shareLink.recipientNote}` : ''}
                  {shareLink.viewCount ? ` · Views: ${shareLink.viewCount}` : ''}
                </p>
              </div>
            ) : (
              <p className="mt-2 text-xs text-slate-600">No active authority share link for this ride.</p>
            )}
            ...
            {shareLink?.url ? (
              <>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-md border border-slate-300 bg-white px-3 py-2 text-center text-xs font-semibold text-slate-700"
                >
                  Open share page
                </a>
                <button
