...
export default function DriversPage() {
  ...
  const exportXlsx = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/admin/drivers/export.xlsx`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: {
      ...
      anchor.href = blobUrl
      anchor.download = `drivers_export_${Date.now()}.xlsx`
      ...
      const apiError = err?.response?.data?.error
      setError(apiError || err?.message || 'Failed to export drivers Excel file')
    }
  }
