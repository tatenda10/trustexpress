import { Router } from 'express';
import usersRouter from './users.js';
import passengersRouter from './passengers.js';
import passengerPaymentsRouter from './passengerPayments.js';
import driverWalletRouter from './driverWallet.js';
import driversRouter from './drivers.js';
import ridesRouter from './rides.js';
import mapsRouter from './maps.js';
import serviceAreasRouter from './serviceAreas.js';
import uploadRouter from './upload.js';
import verifyPhoneRouter from './verifyPhone.js';
import adminAuthRouter from './adminAuth.js';
import adminDriversRouter from './adminDrivers.js';
import adminPassengersRouter from './adminPassengers.js';
import adminRolesRouter from './adminRoles.js';
import adminVehicleTiersRouter from './adminVehicleTiers.js';
import adminVehicleCatalogRouter from './adminVehicleCatalog.js';
import adminPricingRouter from './adminPricing.js';
import adminServiceAreasRouter from './adminServiceAreas.js';
import adminReportsRouter from './adminReports.js';
import adminRidesRouter from './adminRides.js';
import adminSupportRouter from './adminSupport.js';
import adminOverviewRouter from './adminOverview.js';
import adminAgentsRouter from './adminAgents.js';
import adminAgentRewardsRouter from './adminAgentRewards.js';
import adminCaptainRewardsRouter from './adminCaptainRewards.js';
import adminDiscountsRouter from './adminDiscounts.js';
import adminDriverDiscountReimbursementsRouter from './adminDriverDiscountReimbursements.js';
import adminDriverWalletRouter from './adminDriverWallet.js';
import adminPassengerReferralsRouter from './adminPassengerReferrals.js';
import adminRideSafetyPinRouter from './adminRideSafetyPin.js';
import adminNotificationsRouter from './adminNotifications.js';
import adminHireRouter from './adminHire.js';
import adminHireCommissionRouter from './adminHireCommission.js';
import adminHireTypesRouter from './adminHireTypes.js';
import adminSmileCashRouter from './adminSmileCash.js';
import agentAuthRouter from './agentAuth.js';
import agentInvitesRouter from './agentInvites.js';
import agentRecruitmentRouter from './agentRecruitment.js';
import agentRewardsRouter from './agentRewards.js';
import publicShareRouter from './publicShare.js';
import hireRouter from './hire.js';

const router = Router();

router.get('/health', (req, res) => {
  res.json({ ok: true, service: 'trust-express-api' });
});

router.use('/public', publicShareRouter);
router.use('/users', usersRouter);
router.use('/passengers', passengersRouter);
router.use('/passengers/payments', passengerPaymentsRouter);
router.use('/drivers/wallet', driverWalletRouter);
router.use('/drivers', driversRouter);
router.use('/hire', hireRouter);
router.use('/rides', ridesRouter);
router.use('/maps', mapsRouter);
router.use('/service-areas', serviceAreasRouter);
router.use('/upload', uploadRouter);
router.use('/verify-phone', verifyPhoneRouter);
router.use('/admin/auth', adminAuthRouter);
router.use('/admin/drivers', adminDriversRouter);
router.use('/admin/passengers', adminPassengersRouter);
router.use('/admin/passenger-referrals', adminPassengerReferralsRouter);
router.use('/admin/roles', adminRolesRouter);
router.use('/admin/agents', adminAgentsRouter);
router.use('/admin/agent-rewards', adminAgentRewardsRouter);
router.use('/admin/captain-rewards', adminCaptainRewardsRouter);
router.use('/admin/discount-codes', adminDiscountsRouter);
router.use('/admin/driver-discount-reimbursements', adminDriverDiscountReimbursementsRouter);
router.use('/admin/driver-wallet', adminDriverWalletRouter);
router.use('/admin/vehicle-tiers', adminVehicleTiersRouter);
router.use('/admin/vehicle-catalog', adminVehicleCatalogRouter);
router.use('/admin/pricing', adminPricingRouter);
router.use('/admin/service-areas', adminServiceAreasRouter);
router.use('/admin/overview', adminOverviewRouter);
router.use('/admin/reports', adminReportsRouter);
router.use('/admin/rides', adminRidesRouter);
router.use('/admin/ride-safety-pin', adminRideSafetyPinRouter);
router.use('/admin/notifications', adminNotificationsRouter);
router.use('/admin/support', adminSupportRouter);
router.use('/admin/hire', adminHireRouter);
router.use('/admin/hire-commission', adminHireCommissionRouter);
router.use('/admin/hire-types', adminHireTypesRouter);
router.use('/admin/smile-cash', adminSmileCashRouter);
router.use('/agent/auth', agentAuthRouter);
router.use('/agent', agentInvitesRouter);
router.use('/agent', agentRecruitmentRouter);
router.use('/agent', agentRewardsRouter);

export default router;
